...
def test_should_save_and_load_pricing_information():
    ...
    assert model.look_up_product('candy bar') == 0.85
    assert model.look_up_product('carrot') == 0.15
    assert model.look_up_product('peanuts') == 3.25


def test_should_return_zero_if_product_not_found():
    model = PriceModel()

    assert model.look_up_product('apple') == 0.0
...
def test_should_overwrite_product_if_the_same_product_is_added_multiple_times():
    ...
    assert model.look_up_product('orange') == 0.53
